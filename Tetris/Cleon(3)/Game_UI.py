...
count = {"eat": 0, "sleep": 0, "marriage": 0, "car": "", "house": ""}
...
class game:
    ...
    def start_window(self, game, player):
        ...
        game.bind("<Return>", lambda e: self.popup_death(e, game, "health"))
        ...
        if name.isalpha():
            ...
            for i in player.variables:
                ...
                if i == "name":
                    ...
                    bar_stats_frame = tkinter.Frame(window_frame_bottom, name="bar_stats_frame" + i, bg="gray",
                                                    width=400)
                    bar_stats_frame.pack(fill="x", padx="10")
                    ...
                    bar_stats_LBL = tkinter.Label(bar_stats_frame, bg="green", name=i + "bar",
                                                  width=int((int(player.get_value(i)) / 100) * 70))
    ...
    def narration(self, window_frame_top, click_continue, game, window_frame_bottom, player):
        ...
        game.bind('<Button-1>',
                  lambda e: self.main_game_start(narration_frame, click_continue, game, player, window_frame_bottom))
    ...
    def main_game_start(self, narration_frame, click_continue, game, player, window_frame_bottom):
        ...
        year_BTN = tkinter.Button(game_frame, text="Years Later...", width=10, font=("Arial", 20, "bold"),
                                  command=lambda: self.next_year(player, window_frame_bottom,game))
        year_BTN.pack(side="right", anchor="n")

    def next_year(self, player, window_frame_bottom,game):
    ...
    def menu_popup(self, game, player, window_frame_bottom):
        ...
        menu_list = ['eat', 'sleep', 'marriage', 'work', 'car', 'house', 'stats', 'suicide']
        ...
        for menu_list_item in menu_list:
            ...
            elif menu_list_item == "suicide":
                menu_list_BTN = tkinter.Button(frame, name=menu_list_item, text=menu_list_item.capitalize(), width=10,
                font = ("Arial", 18))
                menu_list_BTN["command"] = lambda menu_list_BTN=menu_list_BTN: self.popup_death("1", game, "suicide")
                menu_list_BTN.pack(side="right", fill="both", expand=True)
            elif menu_list_item == "work":
                if int(player.get_value("health")) > 15:
                    menu_list_BTN = tkinter.Button(frame, name=menu_list_item, text=menu_list_item.capitalize(), width=10,
                                                   font=("Arial", 18))
                    menu_list_BTN["command"] = lambda menu_list_BTN=menu_list_BTN: self.menu_sub_popup(popup, menu_list_BTN,
                                                                                                       player,
    ...
    def menu_sub_popup(self, popup, menu_list_BTN, player, window_frame_bottom, game):
        ...
        if "Eat" == menu_list_BTN.cget("text"):
            ...
            if flag == 1:
                ...
                if count["eat"] > 10:
                    textlabel_energy = tkinter.Label(menu_sub_frame, text="You are FULL and you can not eat anymore",
                                                     font=('Arial', 10))
                    textlabel_energy.pack(anchor="n", pady=150, side="top", fill="x")
                else:
                    textlabel_energy = tkinter.Label(menu_sub_frame,
                                                     text="You are BROKE and you can not affort to eat anymore",
                                                     font=('Arial', 10))
                    textlabel_energy.pack(anchor="n", pady=150, side="top")
            ...
            if int(player.get_value("age")) >= 21:
                ...
                textlabel_energy = tkinter.Label(menu_sub_frame, text="You are UNDERAGE!!! too young for Marriage",
                                                 font=('Arial', 15))
                textlabel_energy.pack(anchor="n", pady=150, side="top")
            ...
            # C4X
            textlabel_energy = tkinter.Label(menu_sub_frame,
                                             text="You Signed on as a Competent and strong minded cyber expert in the SAF!!!!",
                                             font=('Arial', 30), wraplengt=500)
            textlabel_energy.pack(anchor="n", pady=100, side="top")

            textlabel_energy = tkinter.Label(menu_sub_frame, text="Cash +$100,000", font=('Arial', 20))
            textlabel_energy.pack(anchor="n", pady=0, side="top")

            player.set_value("money", 100000)

            if int(player.get_value("energy")) >= 15:
                player.set_value("energy", -15)
            else:
                player.set_value("health", -15)
                if int(player.get_value("health")) <= 0:
                    self.popup_death("", game, "health")

        elif "Car" == menu_list_BTN.cget("text"):
            # C4X
            if count["car"] == "":
                textlabel_energy = tkinter.Button(menu_sub_frame, text="Toyota Altis", name="c1", font=('Arial', 20),
                                                  command=lambda: self.carprice(100000, player, menu_sub_frame,
                                                                                "Toyota Altis"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")

                textlabel_energy = tkinter.Button(menu_sub_frame, text="Mercedes Benz A200", name="c2",
                                                  font=('Arial', 20),
                                                  command=lambda: self.carprice(250000, player, menu_sub_frame,
                                                                                "Mercedes Benz A200"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")

                textlabel_energy = tkinter.Button(menu_sub_frame, text="Aston Martin Vantage", name="c3",
                                                  font=('Arial', 20),
                                                  command=lambda: self.carprice(500000, player, menu_sub_frame,
                                                                                "Aston Martin Vantage"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")
            else:
                textlabel_energy = tkinter.Label(menu_sub_frame, text="You owned a " + count["car"], name="c3",
                                                 font=('Arial', 20))
                textlabel_energy.pack(anchor="n", pady=50, side="top")
            ...
            # C4X
            if count["house"] == "":
                textlabel_energy = tkinter.Button(menu_sub_frame, text="HDB", name="c1", font=('Arial', 20),
                                                  command=lambda: self.houseprice(150000, player, menu_sub_frame,
                                                                                  "HDB"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")

                textlabel_energy = tkinter.Button(menu_sub_frame, text="Condo", name="c2", font=('Arial', 20),
                                                  command=lambda: self.houseprice(300000, player, menu_sub_frame,
                                                                                  "Condo"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")

                textlabel_energy = tkinter.Button(menu_sub_frame, text="Landed", name="c3", font=('Arial', 20),
                                                  command=lambda: self.houseprice(1750000, player, menu_sub_frame,
                                                                                  "Landed"))
                textlabel_energy.pack(anchor="n", pady=50, side="top")
            else:
                textlabel_energy = tkinter.Label(menu_sub_frame, text="You owned a " + count["house"], name="c3",
                                                 font=('Arial', 20))
                textlabel_energy.pack(anchor="n", pady=50, side="top")

        elif "Stats" == menu_list_BTN.cget("text"):
            self.stats(player, menu_sub_frame)
        ...
        if "Suicide" != menu_list_BTN.cget("text"):
    ...
    def popup_death(self, e, game, type):
        menu_sub_popup = tkinter.Toplevel(game)
        menu_sub_popup.title("Choices")
        menu_sub_popup.grab_set()
        menu_sub_popup.geometry("500x200")

        if type == "suicide":
            label = tkinter.Label(menu_sub_popup,
                                  text="You chose to suicide and you lived your life with REGRETS......")
            label.pack()
            button = tkinter.Button(menu_sub_popup,
                                    text="Accept your Death", command= lambda: self.destroy(game))
            button.pack()
        else:
            label = tkinter.Label(menu_sub_popup,
                                  text="You chose to live an unhealthy life and died with 0 health......")
            label.pack()
            button = tkinter.Button(menu_sub_popup,
                                    text="Accept your Death", command= lambda: self.destroy(game))
            button.pack()
    ...
    def houseprice(self, value, player, menu_sub_frame, house):
        menu_sub_frame.nametowidget("c1").destroy()
        menu_sub_frame.nametowidget("c2").destroy()
        menu_sub_frame.nametowidget("c3").destroy()
        ...
        if int(player.get_value("money")) >= value:
            count["house"] = house
            label = tkinter.Label(menu_sub_frame, text="You have bought a " + house)
            label.pack(anchor="n", pady=150, side="top")
        else:
            label = tkinter.Label(menu_sub_frame,
                                  text="You have insufficient amount of money to purchase this apartment")
            label.pack(anchor="n", pady=150, side="top")
    ...
    def stats(self, player, menu):
        window_frame_top = tkinter.Frame(menu)
        window_frame_top.pack(fill="x", expand=True)

        window_frame_bottom = tkinter.Frame(menu)
        window_frame_bottom.pack(fill="x", expand=True, side="bottom")
    ...
    def lol(self,game):
        ...
        for i in range(len(df)):
            # print(df.iloc[i])
            label = tkinter.Label(menu_sub_popup, text=str(df.iloc[i][2]))
            label.pack()

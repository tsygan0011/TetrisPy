...
class Player:
    ...
    variables = ['name', 'gender', 'age', 'energy', 'money', 'health', 'iq', 'karma', 'time_limit', 'difficulty']
    variables_limit = [None, None, 99, 100, 999999999, 100, None, 1, 180, None]
    variables_rate = [1, 1]
    ...
    def set_value(self, var_name, amt):
        if not var_name == "name":
            ...
            # this is where you set your name!
            self.personalisation[0] = amt
    ...
    def set_value_limit_by_age(self, var_name):
        ...
        for var in self.variables:
            if var == var_name:
                ...
                # get new limit for var_name passed in
                rate = (self.personalisation[self.variables.index(var)] / (
                    (self.get_age_in_range(age) / self.get_rate_of_consumption(var_name))))
                # print(rate)
